 ...
 * interact with the teiid server through the teiid admin api
 *
...
public class TeiidService /*implements ITeiidService */{/*
	...
	*//**
	...
	public List<DataSourcePageRow> getDataSources(final String filters1, final String srcVdbPrefix) {
		...
		// Get list of all Server Sources (exclude sources that are (1)preview
		// vdbs, (2)test VDBs and (3)service VDBs)
		List<String> filteredDsList = new ArrayList<String>();
		try {
			List<String> allDSList = getDataSourceNames();
			for (String sourceName : allDSList) {
				if (!StringUtils.isEmpty(sourceName) && !sourceName.startsWith(Constants.PREVIEW_VDB_PREFIX)
		...
		for (String dsName : filteredDsList) {
			...
			dataSourcePageRow.setName(dsName);
			dataSourcePageRow.setType(typeList.get(i));
			// If DataSource has a corresponding VDB Source, check connection to
			// the VDB Source
			if (hasSrcVdbList.get(i)) {
				...
				// If DataSource has no corresponding source, the propsPanel
				// will show 'no translator' warning
			} else {
	...
	@Override
	public String testConnection(String sourceName) {
		String sourceJndiName = Constants.JNDI_PREFIX + sourceName;
		return queryService.testConnection(sourceJndiName, sourceName);
	}

	@Override
	public Either<Notification, String> testConnection(TeiidConnection teiidConnection) {
...
//			return left
	...
	private String getTranslatorForSrcVdb(String srcVdbName) {
		...
		String physModelName = srcVdbName.substring(
				srcVdbName.indexOf(Constants.SERVICE_SOURCE_VDB_PREFIX) + Constants.SERVICE_SOURCE_VDB_PREFIX.length());
	...
	private List<TranslatorImportPropertyBean> getImportPropertiesForSrcVdb(String srcVdbName)
	...
	*//**
	...
	public Map<String, List<TranslatorImportPropertyBean>> getImportPropertiesMap(List<String> translators)
			throws DataVirtUiException {
	...
	public List<TranslatorImportPropertyBean> getTranslatorImportProperties(String translatorName)
			throws DataVirtUiException {
	...
	public List<DataSourcePropertyBean> getDataSourceTypeProperties(String typeName) throws DataVirtUiException {
		...
		for (PropertyDefinition propDefn : propDefnList) {
			...
			if (defaultValue != null) {
				propBean.setValue(defaultValue.toString());
				propBean.setOriginalValue(defaultValue.toString());
				// Set Connection URL to template if available and value was
				// null
			...
			// Copy the 'managedconnectionfactory-class' default value into the
			...
			// Resets google 'isRequired' status on some properties. This is
			// because teiid marks them all as
		...
		for (PropertyDefinition pDefn : propDefns) {
			if (pDefn.getName().equalsIgnoreCase(CONN_FACTORY_CLASS_KEY)) {
				...
				if (defaultClassName != null
						&& defaultClassName.equalsIgnoreCase(TranslatorHelper.TEIID_GOOGLE_CLASS)) {
					isGoogle = true;
				}
				break;
			}
	...
	*//**
	...
	public void createDataSourceWithVdb(DataSourceWithVdbDetailsBean bean, int vdbDeployTimeoutSec)
			throws DataVirtUiException {
		// First delete the server source and corresponding vdb source, if they
		// exist
		deleteDataSource(bean.getName());
	...
	*//**
	 ...
	 * 
	 * @param bean
	 *            the DataSource and VDB details
	...
	public void createSourceVdbWithTeiidDS(DataSourceWithVdbDetailsBean bean, int vdbDeployTimeoutSec)
		...
		String sourceVDBStatus = deploySourceVDB(bean.getSourceVdbName(), bean.getName(), bean.getName(), jndiName,
				bean.getTranslator(), importProps, bean.getDDL());

		// Create the teiid dataSource for the deployed source VDB
		// commenting this call as I don't know why to create separate
		// datasource for the VDB since its already created
		...
		for (VDBMetaData vdbMeta : vdbsToRedeploy) {
			...
			try {
				...
				// If the source VDB deployment was not successful, no need to
				// wait the full timeout period for VDB deployment
	...
	*//**
	...
	private String deploySourceVDB(String sourceVDBName, String modelName, String dataSourceName, String jndiName,
		...
		try {
			...
			// Create source model - same name as dataSource. Use model name for
			// source mapping - will be unique
			ModelMetaData udfModel = vdbHelper.createSourceModel(modelName + "UDF", modelName + "UDF", jndiName,
					translator, importProps, Optional.of(ddl));
			...
			sourceVdb.addModel(udfModel);
			sourceVdb.addModel(model);
			sourceVdb.addProperty("lib", "celsiusToFahrenheit");
	...
	*//**
	...
	private String getSourceJndiName(String dataSourceName) throws DataVirtUiException {
		...
		// If jndiName property was not set, assign java:/dsName. Except for the
	...
	*//**
	...
	public void deleteSourcesAndVdbRedeployRenamed(Collection<String> dsNames, String vdbName,
		...
		// Save all VDBS that use the sourceVDB. They will need to be
		// redeployed.
		Collection<VDBMetaData> vdbsToRedeploy = getVdbsWithImport(vdbName);
		// Save exising vdb import and new vdb import names. Import needs to be
	...
	public VdbDetailsBean deployNewVDB(final String vdbName, final int vdbVersion, final Map<String, String> vdbPropMap,
		...
		try {
			...
			if (deployString != null) {
				// Deploys the VDB, waits for it to load and deploys
	...
	public VdbDetailsBean deployCombineVDBs(final String vdbName, final int vdbVersion,
			final Map<String, String> vdbPropMap, final ViewModelRequestBean viewModelRequest)
			throws DataVirtUiException {
		// Create a new VDB
		Properties vdbProperties = new Properties();
		vdbProperties.putAll(vdbPropMap);
		...
		List<Integer> rqdImportVdbVersions = viewModelRequest.getRequiredImportVdbVersions();
		if (rqdImportVdbNames != null && !rqdImportVdbNames.isEmpty()) {
			newVdb = vdbHelper.addCombineVDBImports(newVdb, rqdImportVdbNames, rqdImportVdbVersions);
		}
	...
	private boolean waitForVDBDeploymentToLoad(String deploymentName, int timeoutInSecs) {
		...
		do {
			...
			try {
				vdbMetaData = (VDBMetaData) clientAccessor.getClient().getVDB(vdbName, Integer.parseInt(vdbVersion));
			} catch (AdminApiClientException e) {
			}
			...
			if (vdbMetaData != null) {
				...
				if (vdbMetaData.getModels().isEmpty() || vdbStatus == Status.FAILED || vdbStatus == Status.REMOVED
						|| vdbStatus == Status.ACTIVE) {
					return true;
				}
				// If the VDB Status is LOADING, but a validity error was found
				// - return
	...
	*//**
	...
	private String getServerHost() {
		...
		// If OPENSHIFT_APP_DNS is set, use it. Otherwise get the jboss bind
		// address
		String sHost = System.getenv("OPENSHIFT_APP_DNS");
		if (!StringUtils.isEmpty(sHost)) {
			sHost = Constants.OPENSHIFT_HOST_PREFIX + sHost; // prepend to
																// designate its
																// openshift DNS
																// name
		}
	...
	private DataSourceWithVdbDetailsBean getDSWithVDBDetailsBean(TeiidConnection teiidConnection) {
		DataSourceWithVdbDetailsBean dbVDBBean = new DataSourceWithVdbDetailsBean();
		dbVDBBean.setCreatedBy(getCreatedBy());
		dbVDBBean.setSourceVdbName(getTestConnectionName(teiidConnection));
		dbVDBBean.setTranslator(getTranslator(teiidConnection));
		
		String srcAndJNDIName = getTestConnectionJndiAndSourceName(teiidConnection);
		
		dbVDBBean.setName(srcAndJNDIName);// source name
		dbVDBBean.setJndiName(Constants.JNDI_PREFIX+srcAndJNDIName);
		dbVDBBean.setType(getDatabaseType(teiidConnection));
		dbVDBBean.setDDL(Constants.UDF);
		
		List<DataSourcePropertyBean> dsPropBean = getDataSouceProperties(teiidConnection, srcAndJNDIName);
		dbVDBBean.setProperties(dsPropBean);
		
		return dbVDBBean;
	}

	private String getCreatedBy() {
		return Constants.createdBy;
	}

	private String getTestConnectionJndiAndSourceName(TeiidConnection teiidConnection) {
		StringBuilder srcJndiNameBuilder = new StringBuilder();
		srcJndiNameBuilder.append(Constants.TEST);
		srcJndiNameBuilder.append(Constants.UNDERSCORE);
		srcJndiNameBuilder.append(teiidConnection.getConnectionParameters().get(Constants.CONNECTION_NAME));
		srcJndiNameBuilder.append(System.currentTimeMillis() % 1000);

		return srcJndiNameBuilder.toString();
	}

	private String getTestConnectionName(TeiidConnection teiidConnection) {
		StringBuilder testConnectionNameBuilder = new StringBuilder();
		testConnectionNameBuilder.append(Constants.TEST);
		testConnectionNameBuilder.append(Constants.UNDERSCORE);
		testConnectionNameBuilder.append(teiidConnection.getConnectionParameters().get(Constants.CONNECTION_NAME));
		testConnectionNameBuilder.append(System.currentTimeMillis() % 1000);
		return testConnectionNameBuilder.toString();

	}

	private String getTranslator(TeiidConnection teiidConnection) {
		switch (teiidConnection.getConnectionParameters().get(Constants.CONNECTION_DATABASE_TYPE).toLowerCase()) {
		case TranslatorHelper.MYSQL:
			return TranslatorHelper.MYSQL5;
	...
	private String getDatabaseType(TeiidConnection teiidConnection) {
		switch (teiidConnection.getConnectionParameters().get(Constants.CONNECTION_DATABASE_TYPE).toLowerCase()) {
		case TranslatorHelper.MYSQL:
			return TranslatorHelper.MYSQL.toLowerCase() + "-" + "xa";
	...
	private List<DataSourcePropertyBean> getDataSouceProperties(TeiidConnection teiidConnection, String jndiName) {
		List<DataSourcePropertyBean> dsPropBeanList = new ArrayList<>();
		
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_JNDI_NAME).setValue("java:/"+jndiName));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_POOL_NAME).setValue(jndiName));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_ENABLE).setValue(Constants.STRING_TRUE));
    	
//    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_CONNECTION_URL).setValue(getJDBCConnectionURL(teiidConnection)));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_XA_DRIVER_CLASS).setValue(getXAJDBCDriverClassName(teiidConnection)));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_DRIVER_CLASS).setValue(getJDBCDriverClassName(teiidConnection)));
    	
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_USER_NAME).setValue(teiidConnection.getConnectionParameters().get(Constants.DS_PROP_USER_NAME)));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_PASSWORD).setValue(teiidConnection.getConnectionParameters().get(Constants.DS_PROP_PASSWORD)));
    	
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_MAX_POOL_SIZE_KEY).setValue(Constants.DS_PROP_MAX_POOL_SIZE_VALUE));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_MIN_POOL_SIZE_KEY).setValue(Constants.DS_PROP_MIN_POOL_SIZE_VALUE));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_USE_JAVA_CONTEXT_KEY).setValue(Constants.STRING_TRUE));
    	
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_DATABASE_NAME).setValue(teiidConnection.getConnectionParameters().get(Constants.DS_PROP_DATABASE_NAME)));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_SERVERNAME).setValue(teiidConnection.getConnectionParameters().get(Constants.DS_PROP_SERVERNAME)));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_PORT_NUMBER).setValue(teiidConnection.getConnectionParameters().get(Constants.DS_PROP_PORT_NUMBER)));
    	
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_PREFILL_KEY).setValue(Constants.STRING_FALSE));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_USE_STRICT_MIN).setValue(Constants.STRING_FALSE));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_FLUSH_STRATEGY_KEY).setValue(Constants.DS_PROP_FLUSH_STRATEGY_VALUE));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_IS_SAME_RM_OVERRIDE).setValue(Constants.STRING_FALSE));
    	dsPropBeanList.add(new DataSourcePropertyBean().setName(Constants.DS_PROP_NO_TX_SEPARATE_POOLS).setValue(Constants.STRING_FALSE));
    	
		return dsPropBeanList;
	}
	
	private String getJDBCConnectionURL(TeiidConnection teiidConnection) {
		StringBuilder jdbcConnectionURLBuilder = new StringBuilder();
		switch (teiidConnection.getConnectionParameters().get(Constants.CONNECTION_DATABASE_TYPE).toLowerCase()) {
		case TranslatorHelper.MYSQL:
			jdbcConnectionURLBuilder.append("jdbc:mysql://");
			jdbcConnectionURLBuilder.append(teiidConnection.getConnectionParameters().get(Constants.SERVER));
			jdbcConnectionURLBuilder.append(":");
			jdbcConnectionURLBuilder.append(teiidConnection.getConnectionParameters().get(Constants.PORT));
			jdbcConnectionURLBuilder.append("/");
			jdbcConnectionURLBuilder.append(teiidConnection.getConnectionParameters().get(Constants.CONNECTION_DATABASE));
			return jdbcConnectionURLBuilder.toString();
	...
	private String getJDBCDriverClassName(TeiidConnection teiidConnection) {
		switch (teiidConnection.getConnectionParameters().get(Constants.CONNECTION_DATABASE_TYPE).toLowerCase()) {
		case TranslatorHelper.MYSQL:
			return DriverHelper.MYSQL_DRIVER;
		default:
			return TranslatorHelper.TEIID;
		}
	}
	
	private String  getXAJDBCDriverClassName(TeiidConnection teiidConnection) {
		switch (teiidConnection.getConnectionParameters().get(Constants.CONNECTION_DATABASE_TYPE).toLowerCase()) {
		case TranslatorHelper.MYSQL:
			return DriverHelper.MYSQL_XA_DRIVER;
		default:
			return TranslatorHelper.TEIID;
		}
	}
