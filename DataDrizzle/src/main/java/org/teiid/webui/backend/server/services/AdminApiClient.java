...
//	private AuthenticationProvider authProvider;
    ...
    static {
    	try {
//    		InputStream input = new FileInputStream(ApplicationConstants.teiidServerPropertiesFileName);
    		ClassLoader loader = Thread.currentThread().getContextClassLoader();
//    		Properties props = new Properties();
    		try(InputStream resourceStream = loader.getResourceAsStream(ApplicationConstants.teiidServerPropertiesFileName)) {
    		    prop.load(resourceStream);
    		}
		} catch (IOException e) {
			//logger TODO Auto-generated catch block
			e.printStackTrace();
		}
    }
    ...
    public AdminApiClient(final boolean validating) throws AdminApiClientException {
        ...
        if (this.validating) {
			try {
				...
				int mgmtPort = Integer.parseInt(prop.getProperty("mgmtPort"));
				String mgmtUserName = prop.getProperty("mgmtUserName");
				String mgmtUserPassord = prop.getProperty("mgmtUserPassord");
	        	this.admin = getAdminApi(this.serverHost,mgmtPort,mgmtUserName,mgmtUserPassord);
			} catch (Exception e) {
...
//            discoverAvailableFeatures();
    ...
    *//**
    ...
    private void establishServerHost() {
    	...
    	// load details from properties file
    ...
    *//**
	...
	public VDBMetaData getVDB(String vdbName, int vdbVersion) throws AdminApiClientException {
		...
		for(VDB aVdb : vdbs) {
			...
			if(vdbMeta.getName()!=null && vdbMeta.getName().equalsIgnoreCase(vdbName) && vdbMeta.getVersion().equalsIgnoreCase(String.valueOf(vdbVersion))) {
				vdb = vdbMeta;
				break;
			}
		}
